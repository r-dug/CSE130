...
int main(){
    ...
    char *menu =    "Calculator Menu:\n\t"
                    ...
                    "(7) Factorial a number\n\t"
                    "(8) Raise first number to the power of the next\n\t"
                    "(9) Test if odd (integers only)\n\t"
                    "(0) Exit\n\n";
    ...
    while(1){
        ...
        if (scanf("%d", &selection) == 0){
            ...
            if(selection == 1){
                ...
                int result = is_odd(num1);
                printf("\n\nResult: %d is odd == %s", num1, result ? "True":"False");
                printf("\n\nPress enter to do another calculation");
                while (getchar() != '\n');
                }
            else if(selection == 0){
